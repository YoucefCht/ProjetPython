...
def lot_words_chirac():
  ...
  # Fusionne les deux fichiers pour obtenir un seul dictionnaire avec la somme des fréquences des mots
  ...
  # Sélectionne les 10 mots les plus fréquents
  top_words = sorted_words[:10]
  
  # Écrit les mots les plus fréquents dans un nouveau fichier
...
def nation():
  ...
  # Tri des présidents par nombre de mentions décroissant
...
def climat():
  ...
  # Recherche du président qui a mentionné ces mots en premier
...
def common_words_all_presidents():
  ...
  # Stockage des mots de chaque président
  words_per_president = {president: set() for president in presidents}

  # Lecture des dictionnaires pour chaque président
  for president, files in presidents.items():
      for file_path in files:
          with open(file_path, 'r') as file:
              for line in file:
  ...
  # Trouver l'intersection des mots entre tous les présidents
  common_words = set.intersection(*words_per_president.values())

  # Écriture des mots communs dans un fichier
